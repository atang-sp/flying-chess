...
- **手机当手柄**：升温局可开启多设备模式，每人在自己手机上查看筹码并完成骰子、反应和惩罚干预。
- **可配置终局结算**：升温局可自定义胜者奖惩动作、次数与单位，可选败者梯度，最后一名惩罚最重。
- **事件卡与命运轮盘**：独立卡池支持回合、连续惩罚和骰点触发，可导入/分享 JSON 卡包。
...
- 事件卡可按每 N 回合、连续惩罚或特定骰点触发；临时倍率和玩家绑定会进入后续规则结算。
...
- 进度解锁记忆/问答小游戏机关；四种 `party_v2` 核心惩罚变体始终可用。
