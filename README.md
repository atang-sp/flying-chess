...
- 一键使用默认场景开局，暖场、升温、终局依次可见。全局热度从 0 累积到 100：
  30 进入升温、70 进入终局、100 等当前完整轮结束后结算；切幕和结束都只发生在轮边界。
- 第 3/6 轮与 6/14 分钟的原有切幕条件仍是兜底；热度与轮数/非暂停活跃时间取更靠后的阶段，
  阶段不会回退。原有 20 分钟结束条件继续有效。
- 每位玩家开局有 1 枚可再生气势筹码，最多持有 3 枚，每回合最多使用一枚：可重掷、
  随机二选一，也可在惩罚结算前转嫁、加码或免疫。
- 成功反应增加 2 热度并奖励反应者 1 枚筹码；完成普通惩罚增加 5 热度，完成加码/双倍、
  连锁或双向/多人惩罚还会增加固定热度并为实际参与者各奖励 1 枚筹码。
...
- 每轮轮换一次非当前玩家反应：预测 `1–3` 或 `4–6`，猜中可保留点数或改为
  `7 − 当前点数`；同一次骰子最多改变一次。
- 活跃时长达到 20 分钟或热度达到 100 后完成当前轮，位置最远者胜；并列玩家各掷骰决胜。
- 结算页生成仅在当前设备显示的高光卡，不上传惩罚正文或玩家身份。
- 运行中的升温局不能无损切换玩法；结束或明确重置后返回首页再选择。
- 玩法选择缓存绑定规则集版本；旧的无版本或 `party_v2` Party 缓存会回退经典局，必须重新显式选择 `party_v3`。
- 首页的社区配置、跨局进度和 Party Studio 都是本地优先、显式启用；经典局保持 `classic_v1`，
  不初始化或显示 Party 热度，也不读取这些实验规则。
- 固定公式、贡献分配、权威入口和幂等边界见 [`docs/PARTY_MOMENTUM.md`](docs/PARTY_MOMENTUM.md)。
...
- 进度解锁记忆/问答小游戏机关和额外返场惩罚；四种 `party_v3` 核心惩罚变体始终可用。
...
- v1.8.0 在用户真实选择或切换本局玩法后增加 `mode_selected` 和
  ...
  `ruleset_version`（`classic_v1`/`party_v3`）。
