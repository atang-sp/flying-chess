...
在已检出不可变 `v1.17.1` 标签的仓库根目录构建同一 commit。`ROOM_SERVER_BUILD_SHA` 只能是公开 Git commit 标识：

```bash
docker build -f apps/room-server/Dockerfile \
  --build-arg ROOM_SERVER_VERSION=1.17.1 \
  --build-arg ROOM_SERVER_BUILD_SHA=<公开完整 commit SHA> \
  -t flying-chess-room:1.17.1 .
```
...
node scripts/room-server-release-smoke.mjs \
  ...
  --expected-server-version 1.17.1 \
